...
class ActivityListWallpaper : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // tra ve mot list anh
        if(intent.getIntExtra("id_trend",0)==0&&intent.getStringExtra("title_trend").equals("Featured"))
            getAllDataFeaturedTrending(recyclerView)
        if(intent.getIntExtra("id_trend",0)==1&&intent.getStringExtra("title_trend").equals("Popular"))
            getAllDataPopularTrending(recyclerView)
        if(intent.getIntExtra("id_trend",0)==2&&intent.getStringExtra("title_trend").equals("New"))
            getAllDataNewTrending(recyclerView)
        if(intent.getIntExtra("id_categories",0)!=0&&intent.getStringExtra("name_title_categories")!="") {
    ...
    private fun getAllDataCategoriesListWallpaper(mId:Int,rv:RecyclerView) {
        ApiInterface.Api2.retrofitService2.getAllDataCategoriesListWallpaper(mId).enqueue(object :Callback<MaterialWallpaperCatList>{
    ...
    private fun getAllDataNewTrending(rv: RecyclerView) {
        ApiInterface.Api2.retrofitService2.getAllDataNewTrending().enqueue(object: Callback<MaterialWapaper>{
...
//                    Log.e("lisst",Gson().toJson(response.body()))
    ...
    private fun getAllDataPopularTrending(rv: RecyclerView) {
        ApiInterface.Api2.retrofitService2.getAllDataPopularTrending().enqueue(object :Callback<MaterialWapaper>{
            override fun onResponse(
                call: Call<MaterialWapaper>,
                response: Response<MaterialWapaper>
            ) {
    ...
    private fun getAllDataFeaturedTrending(rv:RecyclerView) {
         ApiInterface.Api2.retrofitService2.getAllDataFeaturedTrending().enqueue(object :Callback<MaterialWapaper>{
             override fun onResponse(
                 call: Call<MaterialWapaper>,
                 response: Response<MaterialWapaper>
             ) {
    ...
    private fun getAllSliderListColor(mId:Int,rv:RecyclerView){
        ApiInterface.Api2.retrofitService2.getAllSliderListColor(mId).enqueue(object :Callback<MaterialWallpaperCatList>{
            override fun onResponse(
                call: Call<MaterialWallpaperCatList>,
                response: Response<MaterialWallpaperCatList>
            ) {
