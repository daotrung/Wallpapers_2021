...
private val retrofit_2 = Retrofit.Builder().addConverterFactory(MoshiConverterFactory.create(moshi)).baseUrl(BASE_URL_2).build()
interface ApiInterface {
    ...
    @GET("api.php")
    fun getAllDataCategoriesListWallpaper(@Query("cat_id")id:Int,@Query("page")page:Int=1):Call<MaterialWallpaperCatList>

    @GET("api.php?color=Space")
    fun getColorModel():Call<MaterialWallpaperColorMain>

    @GET("api.php")
    fun getAllSliderListColor(@Query("color_id")id:Int,@Query("page")page: Int=1):Call<MaterialWallpaperCatList>
